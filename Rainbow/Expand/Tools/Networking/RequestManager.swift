...
public class RequestTaskManager {
    ...
    /// 获取缓存Data
    @discardableResult
    public func cacheData(completion: @escaping (Data)->()) -> KitDataResponse {
        let dataResponse = KitDataResponse(dataRequest: dataRequest!, cache: cache, cacheKey: cacheKey, completionClosure: completionClosure)
        return dataResponse.cacheData(completion: completion)
    }
    /// 响应Data
    public func responseData(completion: @escaping (NetworkValue<Data>)->()) {
        let dataResponse = KitDataResponse(dataRequest: dataRequest!, cache: cache, cacheKey: cacheKey, completionClosure: completionClosure)
        dataResponse.responseData(completion: completion)
    }
    /// 先获取Data缓存，再响应Data
    public func responseCacheAndData(completion: @escaping (NetworkValue<Data>)->()) {
        let dataResponse = KitDataResponse(dataRequest: dataRequest!, cache: cache, cacheKey: cacheKey, completionClosure: completionClosure)
        dataResponse.responseCacheAndData(completion: completion)
    }
    /// 获取缓存String
    @discardableResult
    public func cacheString(completion: @escaping (String)->()) -> KitStringResponse {
        let stringResponse = KitStringResponse(dataRequest: dataRequest!, cache: cache, cacheKey: cacheKey, completionClosure: completionClosure)
        return stringResponse.cacheString(completion:completion)
    }
    /// 响应String
    public func responseString(completion: @escaping (NetworkValue<String>)->()) {
        let stringResponse = KitStringResponse(dataRequest: dataRequest!, cache: cache, cacheKey: cacheKey, completionClosure: completionClosure)
        stringResponse.responseString(completion: completion)
    }
    /// 先获取缓存String,再响应String
    public func responseCacheAndString(completion: @escaping (NetworkValue<String>)->()) {
        let stringResponse = KitStringResponse(dataRequest: dataRequest!, cache: cache, cacheKey: cacheKey, completionClosure: completionClosure)
        stringResponse.responseCacheAndString(completion: completion)
    }
    /// 获取缓存JSON
    @discardableResult
    public func cacheJson(completion: @escaping (Any)->()) -> KitJsonResponse {
        let jsonResponse = KitJsonResponse(dataRequest: dataRequest!, cache: cache, cacheKey: cacheKey, completionClosure: completionClosure)
        return jsonResponse.cacheJson(completion:completion)
    }
    /// 响应JSON
    public func responseJson(completion: @escaping (NetworkValue<Any>)->()) {
        let jsonResponse = KitJsonResponse(dataRequest: dataRequest!, cache: cache, cacheKey: cacheKey, completionClosure: completionClosure)
        jsonResponse.responseJson(completion: completion)
    }
    /// 先获取缓存JSON，再响应JSON
    public func responseCacheAndJson(completion: @escaping (NetworkValue<Any>)->()) {
        let jsonResponse = KitJsonResponse(dataRequest: dataRequest!, cache: cache, cacheKey: cacheKey, completionClosure: completionClosure)
        jsonResponse.responseCacheAndJson(completion: completion)
    }
}
// MARK: - KitBaseResponse
public class KitResponse {
    ...
    ///
    fileprivate func response<T>(response: DataResponse<T>, completion: @escaping (NetworkValue<T>)->()) {
        responseCache(response: response) { (result) in
            completion(result)
        }
    }
    /// isCacheData
    fileprivate func responseCache<T>(response: DataResponse<T>, completion: @escaping (NetworkValue<T>)->()) {
...
// MARK: - KitJsonResponse
public class KitJsonResponse: KitResponse {
    /// 响应JSON
    func responseJson(completion: @escaping (NetworkValue<Any>)->()) {
        dataRequest.responseJSON(completionHandler: { response in
            self.response(response: response, completion: completion)
        })
    }
    fileprivate func responseCacheAndJson(completion: @escaping (NetworkValue<Any>)->()) {
        if cache { cacheJson(completion: { (json) in
            let res = NetworkValue(isCacheData: true, result: Alamofire.Result.success(json), response: nil)
            completion(res)
        }) }
    ...
    /// 获取缓存json
    @discardableResult
    fileprivate func cacheJson(completion: @escaping (Any)->()) -> KitJsonResponse {
...
// MARK: - KitStringResponse
public class KitStringResponse: KitResponse {
    /// 响应String
    func responseString(completion: @escaping (NetworkValue<String>)->()) {
        dataRequest.responseString(completionHandler: { response in
            self.response(response: response, completion: completion)
        })
    }
    @discardableResult
    fileprivate func cacheString(completion: @escaping (String)->()) -> KitStringResponse {
        if let data = CacheManager.default.objectSync(ofType: Data.self, forKey: cacheKey),
            let str = String(data: data, encoding: .utf8) {
            completion(str)
        } else {
    ...
    fileprivate func responseCacheAndString(completion: @escaping (NetworkValue<String>)->()) {
        if cache { cacheString(completion: { str in
            let res = NetworkValue(isCacheData: true, result: Alamofire.Result.success(str), response: nil)
            completion(res)
        })}
...
// MARK: - KitDataResponse
public class KitDataResponse: KitResponse {
    /// 响应Data
    func responseData(completion: @escaping (NetworkValue<Data>)->()) {
        dataRequest.responseData(completionHandler: { response in
            self.response(response: response, completion: completion)
        })
    }
    @discardableResult
    fileprivate func cacheData(completion: @escaping (Data)->()) -> KitDataResponse {
        if let data = CacheManager.default.objectSync(ofType: Data.self, forKey: cacheKey) {
            completion(data)
        } else {
            if openResultLog {
    ...
    fileprivate func responseCacheAndData(completion: @escaping (NetworkValue<Data>)->()) {
        if cache { cacheData(completion: { (data) in
            let res = NetworkValue(isCacheData: true, result: Alamofire.Result.success(data), response: nil)
            completion(res)
        }) }
